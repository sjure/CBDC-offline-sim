...
class IntermediaryNode(Node):
    ...
    def send_transaction(self, payer_node_id, payee_node_id, amount):
        ...
        logger.info(
            f"Online transaction from {payer_node_id} to {payee_node_id} amount {amount}")
    ...
    def offline_withdraw(self, node, withdraw_payment):
        logger.info(
            f"Withdrawal from offline wallet from {withdraw_payment.tx.from_address} to {withdraw_payment.tx.to_address} amount {withdraw_payment.tx.amount}")
        ...
        if (len(payments)):
            ...
            for payment in payments:
                ...
                if not bc.has_transaction(payment.tx) and payment.tx not in self.fradulent_transactions:
                    successfull_add = bc.add_transaction_from_offline(
                        payment.tx)
                    if not successfull_add:
        ...
        if self.is_online:
            if (poisson(1/p.intermediary_failure_rate)):
                ...
                self.ticks_to_online = int(
                    exponential(p.intermediary_recovery_rate))
