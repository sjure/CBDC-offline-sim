    ...
    def __init__(self, **attr):
        ...
        for node in un.nodes():
            ...
            new_nodes = [(i, dict(data=get_new_node(i, self)))
                         for i in mesh.nodes]
            new_network_nodes = [
                (i, dict(data=get_network_node(i, self))) for i in mesh.t2]
        ...
        self.add_nodes_from([(node_dict[i], dict(
            data=get_network_node(node_dict[i], self))) for i in un.nodes()])
        self.add_edges_from([(node_dict[i], node_dict[j])
                            for i, j in un.edges()])
